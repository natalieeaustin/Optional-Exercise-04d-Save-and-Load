...
func save_game():
	...
	save_game.open_encrypted_with_pass(SAVE_PATH, File.WRITE)	# prep it for writing to, make sure the contents are encrypted
...
func load_game():
	...
	if not save_game.file_exists(SAVE_PATH):				# If it doesn't exist, skip the rest of the function
		return
	save_game.open_encrypted_with_pass(SAVE_PATH, File.READ)	# The file should be encrypted
